...
class Order(models.Model):
    ...
    total_price_items = models.IntegerField(default=0)
    total_points_items = models.IntegerField(default=0)

    active_type = models.CharField(
        max_length=13,
        choices=[('item', 'item'), ('point_item', 'point_item'), ('package', 'package')],
        null=True,
        blank=True
    )
    ...
    def update(self):
        if self.active_type == 'item':
            self.total_price_items = sum(order_item.quantity * order_item.item.price for order_item in self.orderitem_set.all())
        elif self.active_type == 'point_item':
            self.total_points_items = sum(order_point_item.quantity * order_point_item.point_item.points for order_point_item in self.orderpointitem_set.all())
